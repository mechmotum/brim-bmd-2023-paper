...
\softwarepackage{FastBike}~\citep{fastbike} simulates single-track vehicles, but remains proprietary, and \softwarepackage{JBike6}~\citep{jbike6} only calculates the eigenvalues of one model.
...
\end{equation}
where $\mathbf{M}$ is the $U \times U$ mass matrix of the bicycle model and $\mathbf{k}$ is its length-U forcing column vector encompassing both externally applied forces and forces due to various velocity effects.
...
In the trajectory tracking optimization problems, the task is to follow a target ground path representing a lane change.
...
\begin{equation}
    ...
    \end{cases} \, ,
\end{equation}
where $q_1$ and $q_2$ are the states describing the $x$ (longitudinal) and $y$ (lateral) position of the rear wheel contact point in the Newtonian coordinate system respectively, $\bar{q}_2$ is the target lateral position as a function of $q_1$, $X_1$ and $X_2$ are the longitudinal length and lateral displacement of the target path respectively, and $X_s$ is the length of straight at the beginning and end of the path.

The objective function $J$ simultaneously minimizes both the tracking error and the input magnitudes, to find controls that minimize the approximated energy used in the maneuver.
It is defined as
\begin{equation}
...
In the optimal controls for all six problems, countersteering near $t_0$ acts to roll the bicycle toward the centerline of the target path while steering the bicycle away from it.
